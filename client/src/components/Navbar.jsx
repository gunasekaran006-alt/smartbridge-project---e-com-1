...
const Navbar = () => {
  ...
  const handleSearch = () => {
    if (categories.includes(productSearch.toLowerCase())) {
    ...
    <>
      ...
      {!usertype ?
        ...
        <div className="navbar">
          <h3 onClick={() => navigate('')}>Kiddy</h3>
          <div className="nav-content">
            <div className="nav-search">
              <input type="text" name="nav-search" id="nav-search" placeholder='Search T-shirts, Sweater, Hats etc.,' onChange={(e) => setProductSearch(e.target.value)} />
              <FcSearch className="nav-search-icon" onClick={handleSearch} />
              {
                noResult === true ?
                  <div className='search-result-data'>no items found.... try searching for T-shirts, Sweater, Hats etc., <ImCancelCircle className='search-result-data-close-btn' onClick={() => setNoResult(false)} /></div>
                  :
                  ""
              }
            </div>
        ...
        : <>
          ...
          {usertype === 'customer' ?
            ...
            <div className="navbar">
              ...
              <div className="nav-content">
                <div className="nav-search">
                  ...
                  {
                    noResult === true ?
                      <div className='search-result-data'>no items found.... T-shirts, Sweater, Hats etc., <ImCancelCircle className='search-result-data-close-btn' onClick={() => setNoResult(false)} /></div>
                      :
                      ""
                  }
                </div>
            ...
            <div className="navbar-admin">
              <h3 onClick={() => navigate('/admin')}>Kiddy (admin)</h3>
